...
int main(void)
{
	...
	while (1) {
		...
		if (audio_block_ready != 0) {
	        ...
	        // Slide window back by one block of real samples
	        memmove(audio_window,
	                audio_window + AUDIO_SAMPLES_PER_BLOCK,
	                (AUDIO_WINDOW_SIZE - AUDIO_SAMPLES_PER_BLOCK) * sizeof(int16_t));

	        // Copy only left channel (even indices) into window
	        for (int i = 0; i < AUDIO_SAMPLES_PER_BLOCK; i++) {
	            audio_window[AUDIO_WINDOW_SIZE - AUDIO_SAMPLES_PER_BLOCK + i] = src[i * 2];
	        }

			blocks_captured++;

			// Window is full after AUDIO_WINDOW_SIZE/AUDIO_BLOCK_SIZE blocks
			// = 16000/512 = 32 blocks = 1 second of audio
			if (blocks_captured >= (AUDIO_WINDOW_SIZE / AUDIO_BLOCK_SIZE) && !dump_done) {
				dump_done = 1;
				for (int i = 0; i < AUDIO_WINDOW_SIZE; i++) {
					printf("%d\r\n", audio_window[i]);
				}
			}
		}
